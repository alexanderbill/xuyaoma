...
connection.query("SET character_set_client=utf8,character_set_connection=utf8,character_set_database=utf8");
...
connection.query(
    ...
    'address VARCHAR(255), '+
    'courses int(255), '+
    'district_ids VARCHAR(255), '+
    'trainingmethod int(255), '+
    'FWAreaValueMax VARCHAR(255), '+
    'title VARCHAR(255), '+
    'counselling int(255), '+
    'description VARCHAR(255), '+
    'category VARCHAR(255), '+
    'citydomainname VARCHAR(255), '+
    'phone1 VARCHAR(255), '+
    ...
    'minor_category int(255), '+
    'jiaojiao_type VARCHAR(255), '+
    'street_ids VARCHAR(255), '+
    'FWAreaValue VARCHAR(255), '+
    'person VARCHAR(255),'+
    'tag int(255),'+
    'PRIMARY KEY (id))'
...
var insertYaoDB = function(query, res){
    var data = {};
    for (var key in query) {
        var ukey = key.replace("[]", "");
        if (para.indexOf(ukey) != -1) {
            if (key == "title" || key == "description" || key == "lession" ||
                key == "honor" || key == "person" || key == "phone1") {
                query[key] = escape(query[key]);
            }
            if (key == "minor_category") {
                var value = query[key];
                var pos = value.indexOf(",");
                if (pos != -1) {
                    query[key] = value.substring(0, pos);
                }
            }
            if (key == "courses" || key == "trainingmethod") {
                if (query[key] instanceof Array) {
                    var total = 0;
                    for (var i in query[key]) {
                        total += (1 << Number(query[key][i]));
                    }
                    data[ukey] = total;
                } else {
                    data[ukey] = (1 << Number(query[key]));
                }
            } else {
                data[ukey] = query[key];
            }
    ...
    debugger;
    connection.query('INSERT INTO ' + DATABASE + ' SET ?', data, function(err, result) {
      if (!err) {
        res.writeHead(200);
...
var selectYaoDB = function(query, res){
    debugger;
    var where = " where ";
    var tt = false;
    if (query.minor_category && query.minor_category != 0) {
        where += "minor_category = " + query.minor_category;
        tt = true;
    }
    else if (query.tag && query.tag != 0) {
        where += "tag = " + query.tag;
        tt = true;
    }
    if (query.trainingmethod && query.trainingmethod != 0) {
        where += (tt ? " AND" : "") + " trainingmethod & " + (1 << Number(query.trainingmethod)) + " <> 0" + "";
        tt = true;
    }
    if (query.courses && query.courses != 0) {
        where += (tt ? " AND" : "") + " courses & " + (1 << Number(query.courses)) + " <> 0";
        tt = true;
    }
    console.log('SELECT * FROM ' + DATABASE + (tt ? where : "") + ' order by created desc');
    connection.query('SELECT * FROM ' + DATABASE + (tt ? where : "") + ' order by created desc', function(err, result){
      debugger;
      var str = {};
      var index = 0;
      result.forEach(function(user){
          user.title = unescape(user.title);
          user.person = unescape(user.person);
          user.lession = unescape(user.lession);
          user.honor = unescape(user.honor);
          user.description = unescape(user.description);
          user.phone1 = unescape(user.phone1);
          str[index++] = user;
      });
      res.writeHead(200);
      res.end(JSON.stringify(str));
      console.log("query " + JSON.stringify(str));
    });
};
...
function Manager (server, options) {
  ...
  server.on('request', function (req, res) {
    ...
    req.addListener("end", function () {
        ...
        var totalQuery = req.postdata;
        if (pos != -1) {
            totalQuery += "&" + req.url.substring(pos + 1);
        }
        var query = querystring.parse(totalQuery);
        if (req.url.indexOf("socket.io/1/") != -1) {
            if (query.act == "pub") {
                insertYaoDB(query, res);
            } else if (query.act == "query") {
                selectYaoDB(query, res);
            }
        }
    });
